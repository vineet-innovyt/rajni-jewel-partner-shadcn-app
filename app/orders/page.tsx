...
export default function OrdersPage() {
  ...
  useEffect(() => {
    if (user) {
      const allOrders = JSON.parse(localStorage.getItem("jewelry_orders") || "[]")
                ...
                >
                  <div className="flex-grow text-left">
                    <div className="flex gap-4 items-start md:items-center flex-col md:flex-row">
                      <div>
                        <p className="text-sm text-muted-foreground">Order #ORD-{order.id}</p>
                        <h3 className="text-lg font-semibold text-foreground mt-1">{formatDate(order.date)}</h3>
                      </div>
                      <div
                    ...
                    <p className="text-sm text-muted-foreground mt-2">{order.items.length} item(s)</p>
                  </div>

                  <div className="text-right flex flex-col items-end gap-2">
                    <p className="text-2xl font-bold text-primary">
                      ${order.totalPrice.toLocaleString("en-US", { maximumFractionDigits: 2 })}
                    </p>
                    <ChevronDown
                      size={20}
                      className={`text-muted-foreground transition ${expandedOrderId === order.id ? "rotate-180" : ""}`}
                    />
                  </div>
                </button>
                    ...
                    {(order as any).shippingInfo && (
                      <div>
                        ...
                        <div className="bg-card border border-border rounded p-4 text-sm text-muted-foreground">
                          <p>
                            {(order as any).shippingInfo.firstName} {(order as any).shippingInfo.lastName}
                          </p>
                          <p>{(order as any).shippingInfo.street}</p>
                          <p>
                            {(order as any).shippingInfo.city}, {(order as any).shippingInfo.state}{" "}
                            {(order as any).shippingInfo.postalCode}
                          </p>
                          <p>{(order as any).shippingInfo.country}</p>
                          <p className="text-foreground mt-3">{(order as any).shippingInfo.email}</p>
                          <p className="text-foreground">{(order as any).shippingInfo.phone}</p>
                        </div>
                      </div>
                    )}
                    ...
                    <div className="bg-card border border-border rounded p-4 space-y-2">
                      <div className="flex justify-between text-sm">
                        ...
                        <span className="text-foreground">
                          ${(order.totalPrice * 0.926).toLocaleString("en-US", { maximumFractionDigits: 2 })}
                        </span>
                      </div>
                      <div className="flex justify-between text-sm">
