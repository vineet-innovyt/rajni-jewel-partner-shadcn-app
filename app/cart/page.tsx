...
export default function CartPage() {
  ...
  const { items, removeFromCart, updateQuantity, getTotalPrice, updateCustomProduct } = useCart()
  ...
  const handleEditCustom = (productId: string, name: string, variant: string) => {
        ...
        {items.length === 0 ? (
          ...
          <div className="grid grid-cols-1 lg:grid-cols-3 gap-8">
            ...
            <div className="lg:col-span-2 space-y-4">
              ...
              {items.map((item) => {
                const isCustom = "isCustom" in item.product && item.product.isCustom
                  ...
                  <div key={item.product.id} className="bg-card border border-border rounded-lg p-4 flex gap-4">
                    ...
                    {isCustom && (
                      <div className="flex-shrink-0 w-24 h-24 rounded-lg overflow-hidden bg-secondary flex items-center justify-center">
                        <span className="text-xs text-muted-foreground text-center px-2">Custom Item</span>
                      </div>
                    )}
                    ...
                    <div className="flex-grow flex flex-col justify-between">
                      {isEditing ? (
                        ...
                        <div>
                          <div className="text-foreground font-semibold">{item.product.name}</div>
                          {isCustom && (
                            <p className="text-sm text-muted-foreground mt-1">Variant: {item.product.variant}</p>
                          )}
                          {!isCustom && (
                            <p className="text-sm text-muted-foreground mt-1">${item.product.price.toLocaleString()}</p>
                          )}
                          {isCustom && <p className="text-sm text-muted-foreground mt-1">Price: Custom Quote</p>}
                          {isCustom && (
                            <button
                              onClick={() => handleEditCustom(item.product.id, item.product.name, item.product.variant)}
                      ...
                      <div className="flex items-center justify-between">
                        ...
                        <div className="text-right">
                          ...
                          {isCustom && <p className="font-semibold text-muted-foreground">—</p>}
                          <button
